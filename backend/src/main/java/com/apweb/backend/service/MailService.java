...
public class MailService {
    ...
    public String getPasswordResetEmailTemplate(String resetLink) {
        return "<!DOCTYPE html>"
                ...
                + "        .greeting { font-size: 22px; font-weight: 700; color: #1a1a1a; margin-bottom: 25px; }"
                + "        .btn { display: inline-block; background-color: #0d6efd; color: #ffffff !important; padding: 16px 35px; border-radius: 12px; text-decoration: none; font-weight: 700; font-size: 16px; margin-top: 25px; box-shadow: 0 4px 12px rgba(13, 110, 253, 0.3); }"
                + "        .notice { font-size: 14px; color: #6c757d; margin-top: 30px; text-align: center; }"
                + "        .footer { background-color: #f8f9fa; padding: 30px; text-align: center; font-size: 12px; color: #999999; border-top: 1px solid #eeeeee; }"
                + "    </style>"
                + "</head>"
                + "<body>"
                ...
                + "        <div class='header'>"
                + "            <h1>SGIM</h1>"
                + "            <p style='margin: 8px 0 0; font-size: 15px; opacity: 0.9; font-weight: 400;'>Recuperación de Contraseña</p>"
                + "        </div>"
                + "        <div class='content'>"
                + "            <div class='greeting'>¿Olvidaste tu contraseña?</div>"
                + "            <p>Hemos recibido una solicitud para restablecer tu contraseña. No te preocupes, puedes crear una nueva haciendo clic en el botón de abajo.</p>"
                + "            "
                + "            <div style='text-align: center; margin-top: 30px;'>"
                + "                <a href='" + resetLink + "' class='btn'>Restablecer Contraseña</a>"
                + "            </div>"
                + "            "
                + "            <p class='notice'>Si no solicitaste este cambio, puedes ignorar este correo de forma segura. El enlace expirará en 24 horas.</p>"
                + "        </div>"
                + "        <div class='footer'>"
                + "            <strong>SGIM - Soluciones Tecnológicas</strong><br>"
                + "            Este mensaje fue generado automáticamente por nuestro sistema.<br>"
