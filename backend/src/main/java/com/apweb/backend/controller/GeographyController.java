...
public class GeographyController {
    ...
    @GetMapping("/ciudades")
    public ResponseEntity<List<Ciudad>> getCiudadesByPais(@RequestParam Integer paisId) {
        List<Ciudad> ciudades = geographyService.getCiudadesByPaisId(paisId);
        return ResponseEntity.ok(ciudades);
    }

    @GetMapping("/cantones")
    public ResponseEntity<List<Canton>> getCantonesByCiudad(@RequestParam Integer ciudadId) {
