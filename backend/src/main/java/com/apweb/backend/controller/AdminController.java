...
public class AdminController {
    ...
    @GetMapping("/users")
    public List<UserAdminView> getAllUsers(@RequestParam(required = false) String role) {
    ...
    @PutMapping("/users/{id}")
    public ResponseEntity<UserAdminView> updateUser(@PathVariable Integer id, @Valid @RequestBody UserUpdateRequest request) {
        UserAdminView updatedUser = adminService.updateUser(id, request);
        return ResponseEntity.ok(updatedUser);
    }

    @PutMapping("/users/{id}/status")
    public ResponseEntity<?> toggleUserStatus(@PathVariable Integer id, @RequestBody Map<String, String> payload) {
        String estado = payload.get("estado");
        if (estado == null || (!estado.equals("ACTIVO") && !estado.equals("INACTIVO"))) {
            return new ResponseEntity<>(Map.of("message", "Invalid status provided. Must be 'ACTIVO' or 'INACTIVO'."), HttpStatus.BAD_REQUEST);
        }
