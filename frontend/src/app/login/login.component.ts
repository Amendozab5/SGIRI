      ...
      this.userService.getUserProfile().subscribe({
        next: (profileData: UserProfileResponse) => {
          ...
          if (mergedUser.primerLogin) {
            ...
            if (mergedUser.roles.includes('ROLE_ADMIN')) {
              this.router.navigate(['/home/admin']);
            } else if (mergedUser.roles.includes('ROLE_TECHNICIAN')) {
    ...
    this.authService.login({ username, password }).pipe(
      ...
      next: (fullUser: User) => {
        ...
        if (fullUser.primerLogin) {
          ...
          if (fullUser.roles.includes('ROLE_ADMIN')) {
            this.router.navigate(['/home/admin']);
          } else if (fullUser.roles.includes('ROLE_TECHNICIAN')) {
            this.router.navigate(['/home/tech']);
          } else {
            this.router.navigate(['/home/user']);
          }
